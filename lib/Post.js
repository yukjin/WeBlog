...
Post.queryDetail=function(option,callback){
    db.query("select id,title,content,category_id,author,date_format(create_time,'%Y-%m-%d %H:%i') as create_time,visit_num,reply_num from article where id=? and author=?",[option.id,option.author],function(err,rows){
        if(err){
            return callback(err);
        }
